(function ($) {
    $.fn.EasyTree = function (options) {
        var defaults = {
            ...
            addable: false,
            i18n: {
                deleteNull: 'Выберите узел для удаления',
                deleteConfirmation: 'Удалить этот узел ?',
                confirmButtonLabel: 'Хорошо',
                editNull: 'Выберите узел для редактирования',
                editMultiple: 'Только один узел может быть отредактирован в одно время',
                addMultiple: 'Выберите узел, чтобы добавить новый узел',
                collapseTip: 'коллапс',
                expandTip: 'расширять',
                selectTip: 'выбрать',
                unselectTip: 'не выбран',
        ...
        $('#success').append('<h4>I am a success callout!</h4>');
        this.each(function () {
            var title;
            var easyTree = $(this);
            ...
            $(easyTree).find('li:has(ul)').addClass('parent_li').find(' > span').attr('title', title);
            $(easyTree).find('li.parent_li > span').parent('li.parent_li').find(' > ul > li').hide('fast');
            ...
            if (options.addable) {
                ...
                $(easyTree).find('.easy-tree-toolbar .create > button').attr('title', options.i18n.addTip).click(function () {
                    ...
                    $(createInput).find('.confirm').click(function () {
                        ...
                        var item = $('<li id="3"><span><span class="glyphicon glyphicon-file"></span><a href="javascript: void(0);">' + $(createInput).find('input').val() + '</a> </span></li>');
                        var roditel;
                        var text = $(createInput).find('input').val();
                        $(item).find(' > span > span').attr('title', options.i18n.collapseTip);
                        $(item).find(' > span > a').attr('title', options.i18n.selectTip);
                        if (selected.length <= 0) {
                            roditel = 0;
                            var id = sent(roditel, text);
                            $(easyTree).find(' > ul').append('<li id="' + id + '"><span><span class="glyphicon glyphicon-file"></span><a href="javascript: void(0);">' + $(createInput).find('input').val() + '</a> </span></li>');
                        } else if (selected.length > 1) {
                            $(easyTree).prepend(warningAlert);
                            $(easyTree).find('.alert .alert-content').text(options.i18n.addMultiple);
                        } else {
                            if ($(selected).hasClass('parent_li')) {
                                roditel = $(selected).attr('id');
                                var id = sent(roditel, text);
                                $(selected).find(' > ul').append('<li id="' + id + '"><span><span class="glyphicon glyphicon-file"></span><a href="javascript: void(0);">' + $(createInput).find('input').val() + '</a> </span></li>');
                            } else {
                                roditel = $(selected).attr('id');
                                $(selected).addClass('parent_li').find(' > span > span').addClass('glyphicon-folder-open').removeClass('glyphicon-file');
                                var id = sent(roditel, text);
                                $(selected).append($('<ul></ul>')).find(' > ul').append('<li id="' + id + '"><span><span class="glyphicon glyphicon-file"></span><a href="javascript: void(0);">' + $(createInput).find('input').val() + '</a> </span></li>');
                            }
                        }
                            ...
                            $(item).find(' > span > a').click(function (e) {
                                ...
                                if (li.hasClass('li_selected')) {
                                    ...
                                    $(this).dblclick(function () {
                                        ...
                                        $(li).addClass('li_selected');
                                        window.location.href = 'http://' + window.location.host + '/DoktorOnix-web/tovary/view/' + title;
                                    });
                                }
                        ...
                        $(editor).keydown(function (e) {
                            if (e.which == 13) {
                                if ($(editor).val() !== '') {
                                    ...
                                    $.ajax({
                                        type: 'GET',
                                        url: 'http://' + window.location.host + '/DoktorOnix-web/tovary/ajax',
                                        data: 'edit=' + id + '&text=' + text + '&add=&remove='
                                    });
                                }
                            }
                        });
            ...
            var sent = function (roditel, text) {
                var id;
                $.ajax({
                    type: 'GET',
                    url: 'http://' + window.location.host + '/DoktorOnix-web/tovary/ajax',
                    data: 'add=' + roditel + '&text=' + text + '&edit=&remove=',
