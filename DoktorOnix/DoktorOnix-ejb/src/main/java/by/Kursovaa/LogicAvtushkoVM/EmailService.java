...
public class EmailService implements EmailServiceRemote {
    ...
    public void sent(by.kursovaa.dbAvtushkoVM.Email mail, MessageBean mess) {
        try {
            ...
            MyAuthent auth = new MyAuthent(mail.getLogin(), mail.getPassword());
    ...
    public void draft(MessageBean Email, by.kursovaa.dbAvtushkoVM.Email mail) {
        try {
            ...
            emailFolder.close(true);
        } catch (GeneralSecurityException | MessagingException | UnsupportedEncodingException ex) {
            Logger.getLogger(Email.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void addAttachments(ArrayList<String> attachments, Multipart multipart)
    ...
    private Store connection(by.kursovaa.dbAvtushkoVM.Email mail)
